...
"""FPGA HDL linting phase via aurig-core's project lint runner.

Sentinel does not parse VHDL itself. The linting phase invokes
``tools/run_lint_project_inprocess.tcl`` from the aurig-core repository
as a subprocess (``tclsh ...``). aurig-core owns the HDL-aware work:
...
------------------------------
aurig-core is cloned manually on the workstation that runs Sentinel and
pointed at via ``phases.linting.tcl4fpga_path`` in the YAML, or via
the env var ``SENTINEL_TCL4FPGA_PATH``. Versioned release tarballs
and single-binary (starkit/tclkit) packaging are tracked as future
work.

Exit code contract (from aurig-core's lint runner)
------------------------------------------------
...
- ``1`` — diagnostics at or above the threshold; Sentinel maps to
  ...
  runs.
- ``2`` — tool error inside aurig-core / tclsh / the subprocess.
...
- Any other non-zero code — treated as a tool/setup issue and
  ...
  aurig-core documents only 0/1/2; an unexpected code means either
  a aurig-core bug, a tclsh crash, or a future contract drift. None
...
TCL4FPGA_RUNNER_REL = ("tools", "run_lint_project_inprocess.tcl")
...
def run_linting(config: Dict[str, Any], ctx: RunContext) -> Optional[Dict[str, Any]]:
    """Run HDL linting via aurig-core's project lint runner.

    Reads ``phases.linting`` from the YAML schema and the top-level
    ``project_manifest``. Invokes
    ``tclsh <aurig-core>/tools/run_lint_project_inprocess.tcl
    ...
    # Resolve tcl4fpga_path: YAML field -> env -> targeted error.
    # An empty string in either source counts as missing (treats
    # `tcl4fpga_path: ""` and `SENTINEL_TCL4FPGA_PATH=` the same way).
    tcl4fpga_path = lint_config.get("tcl4fpga_path") or os.environ.get(
        "SENTINEL_TCL4FPGA_PATH"
    )
    if not tcl4fpga_path:
        logger.error(
            "Linting: tcl4fpga_path not configured "
            "(set phases.linting.tcl4fpga_path or env SENTINEL_TCL4FPGA_PATH)"
        )
        return {
            "status": "error",
            "message": (
                "tcl4fpga_path missing — set phases.linting.tcl4fpga_path "
                "in YAML or env SENTINEL_TCL4FPGA_PATH"
            ),
        }

    tcl4fpga_path = os.path.expanduser(tcl4fpga_path)
    if not os.path.isdir(tcl4fpga_path):
        logger.error(
            "Linting: tcl4fpga_path is not a directory: %s", tcl4fpga_path
        )
        return {
            "status": "error",
            "message": f"tcl4fpga_path not found or not a directory: {tcl4fpga_path}",
        }

    runner_script = os.path.join(tcl4fpga_path, *TCL4FPGA_RUNNER_REL)
    if not os.path.isfile(runner_script):
        logger.error(
            "Linting: aurig-core project runner not found at %s — "
            "confirm tcl4fpga_path points at the aurig-core repository root",
            runner_script,
        )
        return {
            "status": "error",
            "message": (
                "aurig-core lint runner not found at "
                "tools/run_lint_project_inprocess.tcl under tcl4fpga_path "
                f"({tcl4fpga_path}); confirm the path points at the "
                "aurig-core repository root"
            ),
        }
    ...
    # manifest sits under repo_root before handing it to aurig-core.
    ...
    logger.info("Running aurig-core lint: %s", " ".join(cmd))
    ...
    try:
        result = subprocess.run(
            ...
            # path resolution inside aurig-core (anything we don't pass
        ...
        logger.error(
            "aurig-core lint timed out after %s seconds",
    ...
    if rc == 0:
        logger.info(
            "aurig-core lint completed cleanly (no diagnostics meet/exceed fail_on=%s)",
            fail_on,
        )
        return {
            "status": "completed",
    ...
    if rc == 1:
        logger.warning(
            "aurig-core lint reported diagnostics meeting/exceeding fail_on=%s; "
            "phase marked failed",
            fail_on,
        )
        return {
    ...
    if rc == 2:
        logger.error("aurig-core lint reported a tool error (exit 2)")
        return {
            "status": "error",
            "exit_code": 2,
            "error": stderr_tail or "aurig-core reported a tool error",
            "output_dir": output_dir,
            "log_file": combined_log,
            "full_log_file": combined_log,
        }
    # aurig-core's documented contract is 0/1/2 only. An unexpected
    # non-zero code is either a aurig-core bug, a tclsh crash, or a
    ...
    logger.error("aurig-core lint returned unexpected exit code %s", rc)
    return {
        "status": "error",
        "exit_code": rc,
        "error": f"aurig-core returned unexpected exit code {rc}\n{stderr_tail}",
        "output_dir": output_dir,
        "log_file": combined_log,
        "full_log_file": combined_log,
    }
